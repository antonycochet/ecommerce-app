...
interface ICapacityExtraFields {
  handleApplyExtraFields: () => void;
  setData: Dispatch<SetStateAction<IExtraFields>>;
  data: IExtraFields;
}

export default function CapacityExtraFields({
  handleApplyExtraFields,
  setData,
  data,
}: ICapacityExtraFields) {
  ...
  const handleAddInput = (id: number) => {
    let copyArray = data.inputs;
    copyArray.push({ capacity: 0, price: 0, stock: 0, id: id + 1 });
    setData({ ...data, inputs: copyArray });
  };

  const handleUpdateInputValue = (id: number, value: number, type: string) => {
